...
@section('page-title', 'Tambah Perangkat Baru')

@section('content')
    <div class="max-w-2xl mx-auto">
        <div class="mb-6 flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-slate-900">Perangkat Baru</h1>
                <p class="text-slate-500 mt-1 text-sm">Tambahkan data perangkat elektronik ke dalam rumah Anda.</p>
            </div>
        ...
        <div class="bg-white rounded-3xl p-6 md:p-8 shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-slate-100">
            <form method="POST" action="{{ route('devices.store') }}" class="space-y-6">
                ...
                <div class="space-y-2">
                    ...
                    <input id="nama_device" name="nama_device" value="{{ old('nama_device') }}" placeholder="Contoh: AC Kamar Tidur, Kulkas..." required
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            <input id="daya_watt" type="number" name="daya_watt" min="1" value="{{ old('daya_watt') }}" placeholder="Contoh: 400" required
                                   aria-describedby="daya_watt_help"
                                   title="Daya perangkat dalam Watt (W)"
                                   class="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all placeholder:text-slate-400">
                            <span id="daya_watt_help" class="sr-only">Satuan: Watt (W)</span>
                        </div>
                    </div>
