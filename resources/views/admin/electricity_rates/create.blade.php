...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-3xl p-6 md:p-8 shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-slate-100">
        <div class="flex items-center gap-3 mb-6">
            ...
            <div>
                <h3 class="font-bold text-slate-900">Tambah Tarif Listrik Baru</h3>
                <p class="text-xs text-slate-500">Tambahkan tarif berdasarkan daya (VA)</p>
            </div>
        </div>

        <form method="POST" action="{{ route('admin.electricity_rates.store') }}" class="space-y-5">
            @csrf

            <div class="space-y-2">
                <label for="daya_va" class="block text-sm font-semibold text-slate-700">Daya Listrik (VA)</label>
                <div class="relative">
                    <input id="daya_va" type="number" name="daya_va" value="{{ old('daya_va') }}" required
                           placeholder="Contoh: 900"
                           aria-describedby="daya_va_help_create"
                    ...
                    <span id="daya_va_help_create" class="sr-only">Satuan: VA (Volt-Ampere)</span>
                </div>
                @error('daya_va')
            ...
            <div class="space-y-2">
                ...
                <div class="relative">
                    <input id="tarif_per_kwh" type="text" name="tarif_per_kwh" value="{{ old('tarif_per_kwh') }}" required
                           placeholder="Contoh: 1352.00"
                           aria-describedby="tarif_per_kwh_help_create"
                    ...
                    <span id="tarif_per_kwh_help_create" class="sr-only">Mata uang: Rupiah (Rp). Contoh: 1352.00</span>
                </div>
                @error('tarif_per_kwh')
                    <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                @enderror
